...
const bookService = async (bookingData: Partial<TBooking>, bookingUser: { email: string; role: string; }) => {
  const customer = await UserModel.findOne({ email: bookingUser.email, role: bookingUser.role });
  ...
  try {
    ...
    const [booking] = await BookingModel.create([newBookingData], { new: true, session: bookingSession });
    await slot.updateOne({ isBooked: 'booked' }).session(bookingSession);
    await bookingSession.commitTransaction();
    await BookingModel.findById(booking._id).populate('customer').populate('service').populate('slot');
  } catch (error: any) {
...
const getBookings = async (bookingUser: { email: string; role: string; }) => {
  try {
    ...
    if (bookingUser.role === 'admin') {
      return await BookingModel.find().populate('customer').populate('service').populate('slot');
    } else {
      return await BookingModel.find({ customer: customer._id }).populate('customer').populate('service').populate('slot');
    }
  } catch (error: any) {
    throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, error.message,
